#!/bin/bash
# Test the Conventional Commit [Unreleased] lifecycle in generate-changelog.sh
...
non_release_commits='[
  {"type":"docs","section":"Changed","description":"document parser behavior"},
  {"type":"refactor","section":"Changed","description":"simplify parser internals"},
  {"type":"ci","section":"Changed","description":"tighten ci workflow"}
]'
...
release_commits='[
  ...
  {"type":"refactor","section":"Changed","description":"simplify parser internals"},
  {"type":"ci","section":"Changed","description":"tighten ci workflow"},
  {"type":"feat","section":"Added","description":"ship parser support"}
]'
...
monorepo_per_package_no_release='{
  "TMP_PACKAGES/pkg-a": [
    ...
    {"type":"chore","section":"Changed","description":"tidy package a scripts"}
  ],
  "TMP_PACKAGES/pkg-b": [
    {"type":"feat","section":"Added","description":"ship feature for package b"}
  ]
}'
...
monorepo_per_package_release='{
  "TMP_PACKAGES/pkg-a": [
    {"type":"docs","section":"Changed","description":"document package a behavior"},
    {"type":"chore","section":"Changed","description":"tidy package a scripts"},
    {"type":"feat","section":"Added","description":"ship package a feature"}
  ],
  "TMP_PACKAGES/pkg-b": [
    {"type":"feat","section":"Added","description":"ship feature for package b"}
  ]
...
echo "=== Testing Conventional Commit [Unreleased] lifecycle ==="
echo ""
...
run_test "No-release run includes docs entry in [Unreleased]" \
    grep -Fq -- "- document parser behavior" "${changelog_path}"
...
entry_count="$(grep -Fc -- "- document parser behavior" "${changelog_path}" | tr -d '\r')"
...
run_test "Release run carries release-triggering commits into the release entry" \
    grep -Fq -- "- ship parser support" "${changelog_path}"

run_test "Release run clears [Unreleased] content" \
    bash -c 'awk "BEGIN{capture=0} /^## \[Unreleased\]/{capture=1; next} capture && /^## \[/{exit} capture{print}" "$1" | grep -q "^[[:space:]]*$" && ! awk "BEGIN{capture=0} /^## \[Unreleased\]/{capture=1; next} capture && /^## \[/{exit} capture{print}" "$1" | grep -Fq "document parser behavior"' _ "${changelog_path}"

echo ""
echo "=== Testing monorepo Conventional Commit [Unreleased] lifecycle ==="
echo ""

monorepo_dir="${tmp_dir}/packages"
mkdir -p "${monorepo_dir}/pkg-a" "${monorepo_dir}/pkg-b" "${monorepo_dir}/pkg-a-disabled" "${monorepo_dir}/pkg-b-disabled"
